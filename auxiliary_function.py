...
def find_bounding_box(pane, bounding_box_lower_thresholds, bounding_box_upper_thresholds, sort=True):
    ...
    contours, hierarchy = cv2.findContours(
        pane, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    if sort:
        ...
        rect_coordinates = [e for _, e in sorted(
            zip(x_coordinates, rect_coordinates))]
...
def segment_pictures(pane, rect_coordinates, dimension_resized, offset=2):
    ...
    for rec_coordinate in rect_coordinates:
        ...
        resized_pic = np.asarray(Image.fromarray(pane[max(0, y-offset):y+h+offset, max(
            0, x-offset):x+w+offset]).resize((box_resized_width, box_resized_height)))
